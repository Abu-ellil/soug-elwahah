...
export const LocationProvider = ({ children }) => {
  ...
  const updateAvailableStores = async () => {
    try {
      // Fetch ALL stores using search API instead of nearby stores
      // This removes the location restriction
      const response = await API.storesAPI.searchStores('');

      if (response.success) {
        // Normalize store coordinates
        const stores = response.data.stores.map((store) => {
          // Handle GeoJSON format
          let normalizedStore = { ...store };

          // Map _id to id if id is missing
          if (normalizedStore._id && !normalizedStore.id) {
            normalizedStore.id = normalizedStore._id;
          }

          // Normalize categoryId if it's an object (MongoDB format)
          if (normalizedStore.categoryId && typeof normalizedStore.categoryId === 'object' && normalizedStore.categoryId.$oid) {
            normalizedStore.categoryId = normalizedStore.categoryId.$oid;
          } else if (normalizedStore.categoryId && typeof normalizedStore.categoryId === 'object' && normalizedStore.categoryId._id) {
            normalizedStore.categoryId = normalizedStore.categoryId._id;
          }

          if (
            store.coordinates &&
            store.coordinates.coordinates &&
            Array.isArray(store.coordinates.coordinates)
          ) {
          ...
          if (userLocation && normalizedStore.coordinates) {
            normalizedStore.distance = calculateDistance(
              userLocation.lat,
              userLocation.lng,
              normalizedStore.coordinates.lat,
              normalizedStore.coordinates.lng
            );
          }
        ...
        console.error('Failed to get stores:', response.message);
        throw new Error(response.message);
      }
    } catch (error) {
      console.error('Error fetching stores with search:', error);

      // Fallback: Try getNearbyStores with a large radius if user has location
      if (userLocation) {
        try {
          console.log('Attempting fallback to getNearbyStores with large radius...');
          const response = await API.storesAPI.getNearbyStores({
            lat: userLocation.lat,
            lng: userLocation.lng,
            radius: 5000, // 5000 km radius to cover almost everything
          });
          ...
          if (response.success) {
            ...
            const stores = response.data.stores.map((store) => {
              ...
              if (normalizedStore.coordinates && !normalizedStore.distance) {
                normalizedStore.distance = calculateDistance(
                  userLocation.lat,
                  userLocation.lng,
                  normalizedStore.coordinates.lat,
